...
def calibrate_shock_means_const(n, num_sims=40, shock_alpha=0.2):
    ...
    for s in range(num_sims):
        ...
        def obj1(coef):
            means = fit_shock_means(X, Z, Y, t=1, bw=coef * n ** -shock_alpha)
            return mean_squared_error(means, truth1)

        def obj2(coef):
            means = fit_shock_means(X, Z, Y, t=2, bw=coef * n ** -shock_alpha)
...
def calibrate_shock_variance_const(n, num_sims=40, shock_means_const=5., shock_alpha=0.2):
    ...
    for s in range(num_sims):
        ...
        m1 = fit_shock_means(X, Z, Y, t=1, bw=bw1)
        m2 = fit_shock_means(X, Z, Y, t=2, bw=bw1)
